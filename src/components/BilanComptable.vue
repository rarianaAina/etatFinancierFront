...
const fetchIncomeStatement = async () => {
  try {
    const response = await axios.post(
      "http://localhost:8081/api/reports/income-statement",
      {
...
const fetchBalanceSheet = async () => {
  try {
    const response = await axios.post(
      `http://localhost:8081/api/reports/balance-sheet`,
      {
        startDate: startDate.value,
        endDate: endDate.value,
      }
  ...
  <div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="grid grid-cols-2 gap-8">
      ...
      <div>
        ...
        <div
          v-for="(accounts, category) in balanceSheet.assets"
          :key="category"
          class="mb-6"
        >
          ...
          >
            <div class="flex justify-between items-center">
              ...
              <span class="text-gray-600"
                >{{ account.balance.toLocaleString("fr-FR") }} Ariary</span
              >
            </div>
          </div>
        </div>
        <div class="mt-4 pt-2 border-t font-semibold">
          <div class="flex justify-between">
            ...
            <span
              >{{ balanceSheet.totalAssets.toLocaleString("fr-FR") }} Ariary</span
            >
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div
          v-for="(accounts, category) in balanceSheet.liabilities"
          :key="category"
          class="mb-6"
        >
          <h4 class="font-medium text-gray-600 mb-2">{{ category }}</h4>
          ...
          >
            <div class="flex justify-between items-center">
              <span>{{ account.accountName }}</span>
              <span class="text-gray-600"
                >{{ Math.abs(account.balance).toLocaleString("fr-FR") }} Ariary</span
              >
            </div>
          </div>
        </div>
        <div class="mt-4 pt-2 border-t font-semibold">
          <div class="flex justify-between">
            ...
            <span
              >{{
                Math.abs(balanceSheet.totalLiabilities).toLocaleString("fr-FR")
              }}
              Ariary</span
            >
          </div>
        ...
        <div>
          <div class="flex justify-between items-center font-semibold">
            <span>Total passif + Resultat Net</span>
            <span>
              {{
                (
                  Math.abs(balanceSheet.totalLiabilities) +
